...
function UploadContent() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile && selectedFile.type === "text/csv") {
      ...
      setResult({ success: false, message: "Por favor selecciona un archivo CSV válido" })
    }
  }
  ...
  const parseCSV = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      dataLines.forEach((line) => {
        const [route, direction, departureTime, arrivalTime, date, busType] = line.split(",").map((s) => s.trim())

        if (route && direction && departureTime && arrivalTime && date && busType) {
          parsed.push({
      ...
      <header className="bg-slate-900/50 backdrop-blur-xl border-b border-slate-800">
        <div className="container mx-auto px-4 py-3 md:py-4">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
              <h1 className="text-base md:text-xl font-bold text-white">Cargar Servicios</h1>
              <p className="text-xs md:text-sm text-slate-400">Importar malla de servicios desde CSV</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="container mx-auto px-4 py-6 md:py-8 max-w-4xl">
        ...
        <Card className="bg-slate-900/50 border-slate-800 p-4 md:p-8">
          <div className="text-center mb-6">
            ...
            <h2 className="text-xl md:text-2xl font-bold text-white mb-2">Subir archivo CSV</h2>
            <p className="text-sm md:text-base text-slate-400">Selecciona un archivo para cargar los servicios</p>
          </div>
          ...
          <div className="mb-6">
            ...
            >
              <div className="flex flex-col items-center justify-center pt-5 pb-6 px-4">
                ...
                <p className="text-[10px] md:text-xs text-slate-500">Solo archivos CSV</p>
                {file && (
                  <div className="mt-4 px-3 md:px-4 py-2 bg-blue-600/20 rounded-lg max-w-full">
                    <p className="text-xs md:text-sm text-blue-400 font-medium truncate">{file.name}</p>
                  </div>
                )}
              </div>
                  ...
                  {previewData.slice(0, 5).map((service, index) => (
                    ...
                    >
                      <span className="text-slate-300">
                        <strong className="text-white capitalize">{service.route}</strong> - {service.direction} |{" "}
                        {service.date}
                      </span>
          ...
          {result && (
            <div
              className={`mb-6 p-3 md:p-4 rounded-lg flex items-start md:items-center gap-3 ${
                result.success ? "bg-green-600/20 border border-green-600/50" : "bg-red-600/20 border border-red-600/50"
              }`}
            >
              {result.success ? (
                <CheckCircle className="w-4 h-4 md:w-5 md:h-5 text-green-400 flex-shrink-0 mt-0.5 md:mt-0" />
              ) : (
                <AlertCircle className="w-4 h-4 md:w-5 md:h-5 text-red-400 flex-shrink-0 mt-0.5 md:mt-0" />
              )}
              <div>
                <p className={`text-sm md:text-base font-medium ${result.success ? "text-green-400" : "text-red-400"}`}>
                  {result.message}
                </p>
                {result.count && (
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 hover:to-orange-600 text-white font-bold py-4 md:py-6 text-sm md:text-lg rounded-xl shadow-lg shadow-orange-900/50 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isProcessing ? (
              <>
        ...
        <Card className="bg-slate-900/50 border-slate-800 p-4 md:p-6 mt-4 md:mt-6">
          <h3 className="text-base md:text-lg font-bold text-white mb-4">Tipos de Bus Disponibles</h3>
          <div className="grid sm:grid-cols-2 gap-3 md:gap-4">
            <div className="bg-slate-800/50 rounded-lg p-3 md:p-4">
              <h4 className="text-sm md:text-base font-semibold text-blue-400 mb-2">standard-40</h4>
              <p className="text-xs md:text-sm text-slate-400">Bus estándar con 40 asientos (10 filas x 4 asientos)</p>
            </div>
            <div className="bg-slate-800/50 rounded-lg p-3 md:p-4">
              <h4 className="text-sm md:text-base font-semibold text-orange-400 mb-2">ejecutivo-32</h4>
              <p className="text-xs md:text-sm text-slate-400">Bus ejecutivo con 32 asientos (8 filas x 4 asientos)</p>
            </div>
          </div>
        </Card>
      </div>
