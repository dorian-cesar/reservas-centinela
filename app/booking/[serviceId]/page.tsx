...
import { getServices, createReservation, getUserReservations, type BusService } from "@/lib/booking-store"
import { BusSeatLayout } from "@/components/bus-seat-layout"
import { Button } from "@/components/ui/button"
import { Card } from "@/components/ui/card"
import { Bus, MapPin, Calendar, Clock, ArrowLeft, CheckCircle } from "lucide-react"
...
function BookingContent() {
  ...
  const [userReservedSeat, setUserReservedSeat] = useState<number | undefined>(undefined)
  const [showConfirmation, setShowConfirmation] = useState(false)
  const [isLoading, setIsLoading] = useState(false)

  useEffect(() => {
    console.log("[v0] Loading service and checking reservations", { serviceId, userId })

    const services = getServices()
    const foundService = services.find((s) => s.id === serviceId)

    if (!foundService) {
      console.log("[v0] Service not found, redirecting to dashboard")
    ...
    if (userId) {
      ...
      const existingReservation = userReservations.find((r) => r.serviceId === serviceId)
      if (existingReservation) {
        console.log("[v0] User already has reservation:", existingReservation)
  ...
  const handleConfirmReservation = () => {
    console.log("[v0] Confirm reservation clicked", { selectedSeat, userId, service: service?.id })

    if (!selectedSeat && selectedSeat !== 0) {
      console.log("[v0] No seat selected")
      return
    }
    ...
    if (!service) {
      console.log("[v0] No service found")
      return
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-4">
        <Card className="bg-slate-900/50 border-green-700/50 p-8 md:p-12 max-w-md text-center">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-4">Reserva Confirmada</h2>
          <p className="text-sm md:text-base text-slate-300 mb-2">Tu asiento ha sido reservado exitosamente</p>
          <p className="text-green-400 font-semibold text-lg md:text-xl mb-6">
            Asiento #{selectedSeat !== null ? selectedSeat + 1 : ""}
          </p>
          <p className="text-xs md:text-sm text-slate-400">Redirigiendo al dashboard...</p>
        </Card>
      </div>
    )
  }
        ...
        <div className="container mx-auto px-4 py-3 md:py-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg md:text-xl font-bold text-white">Selecciona tu Asiento</h1>
                <p className="text-xs md:text-sm text-slate-400">Elige el asiento de tu preferencia</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 md:gap-8">
          ...
          <div className="order-2 lg:order-1">
            <Card className="bg-slate-900/50 border-slate-800 p-4 md:p-6 mb-4 md:mb-6">
              <h2 className="text-xl md:text-2xl font-bold text-white mb-4">Detalles del Servicio</h2>
              ...
              {selectedSeat !== null && (
                <div className="mt-4 md:mt-6 p-3 md:p-4 bg-orange-600/20 border-2 border-orange-600/50 rounded-xl">
                  <p className="text-xs md:text-sm text-orange-300 mb-1">Asiento Seleccionado</p>
                  <p className="text-2xl md:text-3xl font-bold text-orange-400">#{selectedSeat + 1}</p>
                </div>
              )}
            </Card>
            ...
            {selectedSeat !== null && (
              <Button
                ...
                disabled={isLoading}
                className="w-full bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 hover:to-orange-600 text-white font-bold py-4 md:py-6 text-base md:text-lg rounded-xl shadow-lg shadow-orange-900/50 transition-all duration-300"
              >
            ...
            <div className="mt-4 md:mt-6 p-3 md:p-4 bg-blue-900/20 border border-blue-700/50 rounded-xl">
              <p className="text-xs md:text-sm text-blue-300 font-semibold mb-2">Información Importante:</p>
